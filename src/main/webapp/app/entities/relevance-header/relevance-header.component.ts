...
export default class RelevanceHeader extends mixins(JhiDataUtils) {
  ...
  public mounted(): void {
    this.retrieveAllRelevanceHeaders();
  }
  ...
  public retrieveAllRelevanceHeaders(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.relevanceHeaders.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IRelevanceHeader): void {
  ...
  public removeRelevanceHeader(): void {
    this.relevanceHeaderService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A RelevanceHeader is deleted with identifier ' + this.removeId;
