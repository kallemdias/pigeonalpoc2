...
export default class CheckLine extends mixins(JhiDataUtils) {
  ...
  public retrieveAllCheckLines(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.checkLines.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: ICheckLine): void {
  ...
  public removeCheckLine(): void {
    this.checkLineService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A CheckLine is deleted with identifier ' + this.removeId;
