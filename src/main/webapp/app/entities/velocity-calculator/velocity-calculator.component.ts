...
export default class VelocityCalculator extends mixins(JhiDataUtils) {
  ...
  public velocityCalculators: IVelocityCalculator[] = [];

  public isFetching = false;

  public mounted(): void {
    this.retrieveAllVelocityCalculators();
  }
  ...
  public retrieveAllVelocityCalculators(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.velocityCalculators.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IVelocityCalculator): void {
  ...
  public removeVelocityCalculator(): void {
    this.velocityCalculatorService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A VelocityCalculator is deleted with identifier ' + this.removeId;
