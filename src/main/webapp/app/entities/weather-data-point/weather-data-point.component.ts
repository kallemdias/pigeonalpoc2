...
export default class WeatherDataPoint extends mixins(JhiDataUtils) {
  ...
  public mounted(): void {
    this.retrieveAllWeatherDataPoints();
  }
  ...
  public retrieveAllWeatherDataPoints(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.weatherDataPoints.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IWeatherDataPoint): void {
  ...
  public removeWeatherDataPoint(): void {
    this.weatherDataPointService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A WeatherDataPoint is deleted with identifier ' + this.removeId;
