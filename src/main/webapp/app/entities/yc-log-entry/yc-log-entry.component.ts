...
export default class YcLogEntry extends mixins(JhiDataUtils) {
  ...
  public retrieveAllYcLogEntrys(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.ycLogEntries.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IYcLogEntry): void {
  ...
  public removeYcLogEntry(): void {
    this.ycLogEntryService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A YcLogEntry is deleted with identifier ' + this.removeId;
