...
export default class CheckPoint extends mixins(JhiDataUtils) {
  ...
  public retrieveAllCheckPoints(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.checkPoints.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: ICheckPoint): void {
  ...
  public removeCheckPoint(): void {
    this.checkPointService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A CheckPoint is deleted with identifier ' + this.removeId;
