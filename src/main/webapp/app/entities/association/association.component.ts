...
export default class Association extends mixins(JhiDataUtils) {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.associations.push(res.data[i]);
            }
  ...
  public removeAssociation(): void {
    this.associationService()
      ...
      .then(() => {
        const message = 'A Association is deleted with identifier ' + this.removeId;
