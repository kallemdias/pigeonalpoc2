...
export default class RacingPlan extends mixins(JhiDataUtils) {
  ...
  public retrieveAllRacingPlans(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.racingPlans.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IRacingPlan): void {
  ...
  public removeRacingPlan(): void {
    this.racingPlanService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A RacingPlan is deleted with identifier ' + this.removeId;
